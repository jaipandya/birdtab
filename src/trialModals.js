...
const PRO_FEATURES = [
  {
    ...
    titleKey: 'proFeatureVideoMode',
    titleFallback: 'Video Mode',
    descKey: 'proFeatureVideoModeDesc',
    descFallback: 'Watch birds come alive with stunning videos'
  },
  {
    icon: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    titleKey: 'proFeatureHighRes',
    titleFallback: 'High-Res Photos',
    descKey: 'proFeatureHighResDesc',
    descFallback: 'Crystal clear 2400px images for stunning detail'
  },
  {
    icon: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    titleKey: 'proFeatureRegion',
    titleFallback: 'World Regions',
    descKey: 'proFeatureRegionDesc',
    descFallback: 'Discover birds from any country in the world'
  },
  {
    icon: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <circle cx="12" cy="12" r="10"/>
    ...
    titleKey: 'proFeatureClock',
    titleFallback: 'Clock & Timer',
    descKey: 'proFeatureClockDesc',
    descFallback: 'Keep track of time while enjoying nature'
  }
];
...
export function showTrialWelcomeModal(daysRemaining = 14) {
  ...
  const featuresHTML = PRO_FEATURES.map(f => `
    <div class="trial-feature">
      ...
      <span class="trial-feature-title">${chrome.i18n.getMessage(f.titleKey) || f.titleFallback}</span>
    </div>
  `).join('');

  const daysRemainingText = daysRemaining === 1
    ? (chrome.i18n.getMessage('trialDayLeft') || '1 day left')
    : (chrome.i18n.getMessage('trialDaysLeft', [String(daysRemaining)]) || `${daysRemaining} days remaining`);
  ...
  const modalHTML = `
    <div id="trial-welcome-modal" class="trial-modal" role="dialog" aria-modal="true" aria-labelledby="trial-welcome-title">
      ...
      <div class="trial-modal-content">
        <button class="trial-modal-close" aria-label="${chrome.i18n.getMessage('close') || 'Close'}">
        ...
        <div class="trial-header">
          <div class="trial-badge">
            ...
            <span>${chrome.i18n.getMessage('proTrialActive') || 'Pro Trial Active'}</span>
          </div>
          <h2 id="trial-welcome-title" class="trial-title">
            ${chrome.i18n.getMessage('trialWelcomeTitle') || 'Welcome to BirdTab Pro'}
          </h2>
          <p class="trial-subtitle">
            ${chrome.i18n.getMessage('trialWelcomeSubtitle', [String(daysRemaining)]) || `Your <strong>${daysRemaining}-day free trial</strong> has started. Enjoy all Pro features and discover the full beauty of birds worldwide.`}
          </p>
        </div>
        ...
        <div class="trial-timeline">
          ...
          <div class="trial-timeline-labels">
            <span>${chrome.i18n.getMessage('today') || 'Today'}</span>
        ...
        <div class="trial-actions">
          <button class="trial-btn-primary" id="trial-welcome-start">
            ${chrome.i18n.getMessage('startExploring') || 'Start Exploring'}
        ...
        <p class="trial-footer-note">
          ${chrome.i18n.getMessage('trialFooterNote') || 'No credit card required. After your trial, you\'ll return to the free version.'}
        </p>
      </div>
    </div>
  `;
...
export function showTrialExpiredModal() {
  ...
  const lostFeaturesHTML = PRO_FEATURES.map(f => `
    <div class="trial-lost-feature">
      ...
      <span>${chrome.i18n.getMessage(f.titleKey) || f.titleFallback}</span>
    </div>
  `).join('');
      ...
      <div class="trial-modal-content">
        ...
        <div class="trial-header">
          <div class="trial-badge">
            ...
            <span>${chrome.i18n.getMessage('trialEnded') || 'Trial Ended'}</span>
          </div>
          <h2 id="trial-expired-title" class="trial-title">
            ${chrome.i18n.getMessage('trialExpiredTitle') || 'Your Free Trial Has Ended'}
          </h2>
          <p class="trial-subtitle">
            ${chrome.i18n.getMessage('trialExpiredSubtitle') || 'Thank you for trying BirdTab Pro! You can continue using the core features for free, or upgrade to keep enjoying the full experience.'}
          </p>
        </div>

        <div class="trial-lost-section">
          <p class="trial-lost-heading">${chrome.i18n.getMessage('trialLostFeatures') || 'Pro features you\'ll miss:'}</p>
        ...
        <div class="trial-value-prop">
          <p>
            ${chrome.i18n.getMessage('trialValueProp') || '<strong>Upgrade to Pro</strong> and support independent development while enjoying all features. Your subscription keeps BirdTab ad-free and constantly improving.'}
          </p>
        </div>

        <div class="trial-actions">
          <a href="${PRICING_URL}" target="_blank" class="trial-btn-primary">
            ${chrome.i18n.getMessage('upgradeToPro') || 'Upgrade to Pro'}
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <line x1="5" y1="12" x2="19" y2="12"/>
              <polyline points="12 5 19 12 12 19"/>
            </svg>
          ...
          <button class="trial-btn-secondary" id="trial-expired-continue">
            ${chrome.i18n.getMessage('continueWithFree') || 'Continue with Free'}
          </button>
        </div>
      </div>
    </div>
