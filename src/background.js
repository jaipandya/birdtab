...
// Fetch bird info by specific species code (for loading from history)
async function fetchBirdInfoBySpeciesCode(speciesCode, videoMode = false) {
  log(`Fetching bird info for species: ${speciesCode}, videoMode: ${videoMode}`);
  const startTime = Date.now();
  addBreadcrumb(`Fetching bird by species code: ${speciesCode}`, 'http', 'info');

  try {
    // Get region from storage to fetch correct bird list
    const result = await chrome.storage.sync.get(['region']);
    const region = result.region || 'US';

    const birds = await getBirdsByRegion(region);
    const bird = birds.find(b => b.speciesCode === speciesCode);

    if (!bird) {
      throw new Error(`Bird with species code ${speciesCode} not found in region ${region}`);
    }
    ...
    let imageInfo, audioInfo, videoInfo;
    try {
      // Fetch media (same logic as fetchBirdInfo)
      const fetchPromises = [getMacaulayImage(bird.speciesCode)];
      ...
      if (!videoMode) {
        ...
        fetchPromises.push(
          getMacaulayAudio(bird.speciesCode).catch(error => {
            log(`[AUDIO ERROR]: ${error.message} for species ${bird.speciesCode}`);
            return null;
          })
        );
        fetchPromises.push(Promise.resolve(null)); // placeholder for video
        ...
        fetchPromises.push(
          getMacaulayVideo(bird.speciesCode).catch(error => {
            log(`[VIDEO ERROR]: ${error.message} for species ${bird.speciesCode}`);
            return null;
          })
        );
      }
      ...
      // Image fetch failed - throw error
      log(`Error fetching media for species ${speciesCode}: ${error.message}`);
      throw error;
    }
    ...
    log(`Bird info compiled for ${speciesCode}: ${birdInfo.name}`);

    const duration = Date.now() - startTime;
    addBreadcrumb('Bird info fetched by species code', 'http', 'info', {
      duration,
      speciesCode: birdInfo.speciesCode
    });
    ...
    log(`Error in fetchBirdInfoBySpeciesCode: ${error.message}`);
    captureException(error, {
      tags: { operation: 'fetchBirdInfoBySpeciesCode', component: 'background' },
      extra: { speciesCode, videoMode }
    });
    throw error;
  }
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  } else if (request.action === 'getBirdInfoBySpeciesCode') {
    log(`Received request for bird info by species code: ${request.speciesCode}`);
    startKeepAlive();

    (async () => {
      try {
        // Get video mode from storage
        const result = await chrome.storage.sync.get(['videoMode']);
        const videoMode = result.videoMode || false;

        const birdInfo = await fetchBirdInfoBySpeciesCode(request.speciesCode, videoMode);

        log(`Sending bird info response for species ${request.speciesCode}: ${birdInfo.name}`);
        ...
        try {
          ...
          log(`Failed to send response: ${responseError.message}`);
          captureException(responseError, {
            tags: { operation: 'sendResponse', source: 'getBirdInfoBySpeciesCode' },
            extra: { speciesCode: request.speciesCode }
          });
        }
      } catch (error) {
        log(`Error in getBirdInfoBySpeciesCode handler: ${error.message}`);
        captureException(error, {
          tags: { operation: 'getBirdInfoBySpeciesCode', source: 'messageListener' },
          extra: { speciesCode: request.speciesCode }
        });
